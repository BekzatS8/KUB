...
--
-- NB: миграции гоняются целиком при каждом деплое. К этому моменту в таблице
-- уже могут быть строки с типами, добавленными более поздней миграцией 079
-- (pending_send_document / pending_review_document). Поэтому constraint здесь
-- ставится как NOT VALID — Postgres НЕ сканирует существующие строки и не падает
-- на них; финальный валидный constraint с полным списком выставляет 079.
