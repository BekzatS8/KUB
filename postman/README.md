...
2) Заполните переменные окружения: `baseUrl`, `email`, `password`. `documentId` заполняется автоматически после Create Document (или вручную из List Documents).
...
5) Debug -> Latest Sign Tokens (dev only) — заполнит `emailCode`, `emailToken`, `tgCallbackToken`.
6) Signing -> Confirm Email Code.
7) Signing -> Verify Email Token (если сервер делает redirect — ориентируйтесь на HTTP код ответа).
...
- `emailCode` — OTP-код из письма (email_code).
