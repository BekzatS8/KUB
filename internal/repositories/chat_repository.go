...
`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&chat.ID, &chat.Name, &chat.IsGroup, &chat.CreatedAt, &members, &lastText, &lastAt, &online, &lastSeen, &unreadCount); err != nil {
			return nil, err
		}
...
VALUES ($1, $2, NOW())
ON CONFLICT (user_id) DO UPDATE SET online = EXCLUDED.online, last_seen = NOW()
`
...
func (r *chatRepository) GetOnlineStatus(userID int) (bool, time.Time, error) {
	const q = `
SELECT online, last_seen FROM user_status WHERE user_id = $1
`
	var (
		online   bool
		lastSeen sql.NullTime
...
VALUES ($1, $2, $3)
ON CONFLICT (chat_id, user_id) DO UPDATE SET last_read_message_id = EXCLUDED.last_read_message_id
`
...
`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&chat.ID, &chat.Name, &chat.IsGroup, &chat.CreatedAt, &members, &lastText, &lastAt, &online, &lastSeen, &unreadCount, new(int), new(int)); err != nil {
			return nil, err
		}
		for _, m := range members {
			chat.Members = append(chat.Members, int(m))
...
`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&msg.ID, &msg.ChatID, &msg.SenderID, &msg.Text, &attachmentsBytes, &msg.CreatedAt, new(int)); err != nil {
			return nil, err
		}
		if len(attachmentsBytes) > 0 {
			_ = json.Unmarshal(attachmentsBytes, &msg.Attachments)
