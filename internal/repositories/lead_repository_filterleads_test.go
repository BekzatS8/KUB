...
func (r *leadFilterCheckRows) Columns() []string {
	return []string{"id", "title", "description", "phone", "source", "created_at", "owner_id", "branch_id", "branch_name", "department_id", "funnel_id", "stage_id", "status", "is_archived", "archived_at", "archived_by", "archive_reason"}
}
...
func (r *leadFilterCheckRows) Next(dest []driver.Value) error {
	...
	row := []driver.Value{1, "t", "d", "7700", "web", now, 10, 20, "Main", nil, nil, nil, "new", false, nil, nil, ""}
	for i := range dest {
		dest[i] = row[i]
	}
	return nil
