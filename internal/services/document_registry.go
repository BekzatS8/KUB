...
var documentTypeRegistry = map[string]DocumentTypeSpec{
	...
	"cancel_appointment":        mkDocx("cancel_appointment", "Заявление на отмену записи", "cancel_appointment.docx", baseRequired(), extraKeys(mandatory("reason_code"), optional("CANCEL_REASON_TEXT"), optional("DESTINATION_PLACE"), optional("APPOINTMENT_DATE_TEXT"), optional("CANCEL_OTHER_TEXT")), map[string]string{"reason_code": "R1", "CANCEL_REASON_TEXT": "Личные обстоятельства"}),
	...
	"documents_handover_act":    mkDocx("documents_handover_act", "Акт приёма-передачи документов", "documents_handover_act.docx", baseRequired(), extraKeys(optional("DOCS_ITEMS_JSON"), optional("ACT_NUMBER"), optional("DOCS_MARK_1"), optional("DOCS_MARK_2"), optional("DOCS_MARK_3")), map[string]string{"DOCS_ITEMS_JSON": "[{\"name\":\"Паспорт\",\"mark\":\"☑\"}]"}),
	...
	"contract_paid_50_50_ru":    mkDocx("contract_paid_50_50_ru", "Договор 50/50 (рус)", "contract_paid_50_50_ru.docx", baseRequired(), extraKeys(optionalDealFallback("PREPAY_AMOUNT_NUM"), optionalDealFallback("PREPAY_AMOUNT_TEXT"), optionalDealFallback("CONTRACT_NUMBER"), optionalDealFallback("CONTRACT_DATE_TEXT")), map[string]string{"PREPAY_AMOUNT_NUM": "100000", "PREPAY_AMOUNT_TEXT": "Сто тысяч тенге"}),
	"contract_free_ru":          mkDocx("contract_free_ru", "Договор (бесплатный, рус)", "contract_free_ru.docx", baseRequired(), nil, nil),
	"contract_paid_full_ru":     mkDocx("contract_paid_full_ru", "Договор полной оплаты (рус)", "contract_paid_full_ru.docx", baseRequired(), nil, nil),
	"contract_language_courses": mkDocx("contract_language_courses", "Договор на языковые курсы", "contract_language_courses.docx", baseRequired(), extraKeys(optional("COURSE_NAME"), optionalDealFallback("CONTRACT_NUMBER"), optionalDealFallback("CONTRACT_DATE_TEXT")), map[string]string{"COURSE_NAME": "General English"}),
	"addendum_korea":            mkDocx("addendum_korea", "Дополнительное соглашение (Корея)", "addendum_korea.docx", baseRequired(), nil, nil),
	"refund_application":        mkDocx("refund_application", "Заявление на возврат", "refund_application.docx", baseRequired(), extraKeys(mandatory("reason_code"), optional("REFUND_REASON_TEXT"), optionalDealFallback("REFUND_AMOUNT_NUM"), optionalDealFallback("REFUND_AMOUNT_TEXT")), map[string]string{"reason_code": "R1", "REFUND_REASON_TEXT": "Отказ в визе", "REFUND_AMOUNT_NUM": "100000", "REFUND_AMOUNT_TEXT": "Сто тысяч тенге"}),
