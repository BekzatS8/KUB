...
// TestCharacterize_SalesLeadScope_IsBranch confirms sales sees leads of its own
// branch only (разделение по филиалам, обратная связь 22.08.2026).
func TestCharacterize_SalesLeadScope_IsBranch(t *testing.T) {
	...
	if scope.Kind != ScopeKindBranch || scope.BranchID == nil || *scope.BranchID != branchID {
		t.Errorf("sales lead scope: want ScopeKindBranch(%d), got %+v", branchID, scope)
	}
}

// TestCharacterize_VisaLeadScope_IsBranch confirms visa sees own-branch leads only.
func TestCharacterize_VisaLeadScope_IsBranch(t *testing.T) {
	...
	if scope.Kind != ScopeKindBranch || scope.BranchID == nil || *scope.BranchID != branchID {
		t.Errorf("visa lead scope: want ScopeKindBranch(%d), got %+v", branchID, scope)
	}
}
