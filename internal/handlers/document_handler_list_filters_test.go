...
func TestDocumentListFilterFromQuery_ParsesExpectedFields(t *testing.T) {
	...
	c.Request = httptest.NewRequest("GET", "/documents?q=contract&status=signed&doc_type=addendum_korea&deal_id=25&client_id=16&client_type=individual&sort_by=doc_type&order=asc", nil)
