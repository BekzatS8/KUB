...
func TestClientCreateInvalidClientTypeReturns400(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	body := `{"client_type":"bad","last_name":"Иванов","first_name":"Иван","phone":"+7 777 000 00 00","country":"KZ","trip_purpose":"tourism","birth_date":"1990-01-01"}`
	req := httptest.NewRequest(http.MethodPost, "/clients", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
	...
	if !strings.Contains(w.Body.String(), "invalid client_type") {
		t.Fatalf("expected invalid client_type message, got body=%s", w.Body.String())
	}
}
