...
func SetupRoutes(
	...
	docs := r.Group("/documents")
	{
		...
		// предпросмотр шаблона (docx с подписями полей) — всем, кто видит документы
		docs.GET("/types/:doc_type/preview", middleware.RequirePermission("documents.view", "document"), documentHandler.PreviewDocumentType)
	...
	)
	{
		...
		tasks.POST("/:id/restore", middleware.RequirePermission("tasks.delete", "task"), taskHandler.Restore)
		tasks.DELETE("/:id/purge", middleware.RequirePermission("tasks.delete", "task"), taskHandler.Purge)
	...
	if managerReportHandler != nil {
		reportTables := r.Group("/reports/table")
		{
			...
			reportTables.POST("/my/:id/restore", managerReportHandler.RestoreMy)
			reportTables.DELETE("/my/:id/purge", managerReportHandler.PurgeMy)
			reportTables.GET("", managerReportHandler.List)
			// корзина админа — все удалённые отчёты
			reportTables.GET("/trash", managerReportHandler.ListTrash)
			reportTables.GET("/user/:id", managerReportHandler.ListByUser)
			reportTables.GET("/report/:id", managerReportHandler.GetReport)
			reportTables.POST("/report/:id/restore", managerReportHandler.RestoreReport)
			reportTables.DELETE("/report/:id/purge", managerReportHandler.PurgeReport)
