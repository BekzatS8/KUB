...
|-------------------------|-------------|-----------------------------------------------------|
| `BINOTEL_WEBHOOK_SECRET`| Рекомендуется | Секрет для проверки входящих webhook-запросов. Если не задан — webhook принимает любые запросы (только для dev). |
| `BINOTEL_API_KEY`       | Нет         | API-ключ Binotel (зарезервирован для будущих исходящих вызовов) |
| `BINOTEL_API_SECRET`    | Нет         | API-секрет Binotel (зарезервирован)                 |
