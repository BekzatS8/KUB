...
| B. Create individual client | `POST /clients`, `GET /clients/:id` | Management/Operations/Control/Backoffice (not read-only/admin-staff) | JWT, clients tables | 201 + client with `individual_profile` |
...
| D. Create lead/deal with client | `POST /leads`, conversion endpoints, `GET /deals/:id` | Role per lead/deal policy | clients, leads, deals tables | deal references valid `client_id` |
...
| G. Wazzup outbound | `POST /integrations/wazzup/setup`, `POST /integrations/wazzup/send` | Management/AdminStaff for setup, authenticated for send | Wazzup config, repo integration row, provider API | setup/send returns success or controlled provider error |
